...
class tester:
    ...
    def pause(self, seconds):
        print 'pausing for %d second(s)' % seconds
        time.sleep(seconds)
    ...
    def pre_fan(self, seconds):
        print 'running fans for', seconds, 'seconds to normalise readings'
    ...
    def open_graph(self):
        print 'opening graph'
        self.temp_ax    = plt.subplot2grid((11, 1), (1, 0), rowspan = 4)
        self.hum_ax     = plt.subplot2grid((11, 1), (5, 0), rowspan = 4, sharex=self.temp_ax)
        self.signals_ax = plt.subplot2grid((11, 1), (9, 0), rowspan = 2, sharex=self.temp_ax)
    ...
    def plot(self):
        ...
        self.temp_ax.legend(iter(self.temp_ax.lines), ('Chamber Top', 'Chamber Bottom',
                                                       'Heater Chamber Top', 'Heater Chamber Bottom',
                                                       'Extrernal', 'Humidity Sensor'),
                            bbox_to_anchor=(0.0 ,1.04, 1.0, 0.102), loc=3, ncol=3, mode='expand', borderaxespad=0.0)
        ...
        total_width = 10
        space = 2
        centre_points = [total_width*0.5, total_width*1.5, total_width*2.5, total_width*3.5]
        self.signals_ax.broken_barh( bar_sets[0], (3*total_width+space/2, total_width-space), edgecolor='none', facecolors='red'  ) #heat
        self.signals_ax.broken_barh( bar_sets[1], (2*total_width+space/2, total_width-space), edgecolor='none', facecolors='blue' ) #mist
        self.signals_ax.broken_barh( bar_sets[2], (1*total_width+space/2, total_width-space), edgecolor='none', facecolors='cyan' ) #flap
        self.signals_ax.broken_barh( bar_sets[3], (0*total_width+space/2, total_width-space), edgecolor='none', facecolors='grey' ) #fans
        ...
        if gap == 1: gap = 2
        self.temp_ax.yaxis.set_ticks(range(av-half*gap, av+half*gap+1, gap))
        self.temp_ax.set_ylim(av-(half+0.5)*gap, av+(half+0.5)*gap)
        tks     = self.temp_ax.get_yticks()
        self.temp_ax.set_ylim(int(tks[0])-0.5*gap, int(tks[-1])+0.5*gap)
        ...
        gap     = int(tks[1]) - int(tks[0])
        if gap == 1: gap = 2
        self.hum_ax.yaxis.set_ticks(range(av-half*gap, av+half*gap+1, gap))
        tks     = self.hum_ax.get_yticks()
        self.hum_ax.set_ylim(int(tks[0])-0.5*gap, int(tks[-1])+0.5*gap)
